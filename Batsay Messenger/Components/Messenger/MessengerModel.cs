...
internal class MessengerModel
{
	...
	{
		return Conversations.FirstOrDefault(x => x.Id == selectedConversationId)?.Messages;
	}
	...
	private async Task<Conversation> LoadConversationInfo(long id)
	{
		...
		if (conversationData is not null)
		{
	...
	{
		Conversations.Remove(Conversations.First(x => x.Id == id));
	}
	...
	{
		await Task.Factory.StartNew(() => LongPoll(new Progress<VkNet.Model.Message>(ReceiveMessage)), _token);
	}
	...
	private void LongPoll(IProgress<VkNet.Model.Message> progress)
	{
		...
		while (!_token.IsCancellationRequested)
		{
			...
			if (poll.Updates is null) continue;
